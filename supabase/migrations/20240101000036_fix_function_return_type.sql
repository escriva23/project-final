...
-- Need to drop the existing function before recreating with different return type

-- Drop the existing function first
DROP FUNCTION IF EXISTS update_mtaa_shares_ranks();
...
DECLARE
    ...
    users_count INTEGER;
    rec RECORD;  -- Declare the loop variable
BEGIN
...
-- Now create the ranks function with better logging (after dropping the old one)
CREATE FUNCTION update_mtaa_shares_ranks()
