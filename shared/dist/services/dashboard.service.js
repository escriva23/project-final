...
class DashboardService {
     ...
     */
    async calculateCustomerStatsManually(userId) {
        try {
            // Get booking stats
     ...
     */
    async calculateProviderStatsManually(userId) {
        try {
            // Get booking stats
            const { data: bookings, error: bookingsError } = await this.supabase
                .from('bookings')
     ...
     */
    async getRecentActivity(userId, userType, limit = 10) {
        try {
     ...
     */
    async getEarningsSummary(userId, period = 'month') {
        try {
